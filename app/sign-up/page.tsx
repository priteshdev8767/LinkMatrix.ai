...
export default function SignUp() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (authError) {
      ...
      // Check if user is immediately logged in
      if (data?.accessToken) {
        await checkUser();
        router.push("/dashboard");
      } else {
        setMessage("Account created! Please check your inbox for a verification email.");
        setEmail("");
        setPassword("");
        setConfirmPassword("");
        setLoading(false);
      ...
      <div className="w-full max-w-md z-10 transition-all duration-300">
        ...
        <div className="bg-[#0b0720]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-[0_0_50px_rgba(139,92,246,0.15)]">
          ...
          <div className="mt-8 text-center text-sm text-slate-400">
            ...
            <Link href="/sign-in" className="text-purple-400 hover:text-purple-300 font-semibold transition-colors duration-150">
              Sign In
            </Link>
          </div>
        </div>
